...
class GenericBloomFilter_1 {
public:

  // Constructors
  // GenericBloomFilter_1(string config);
  GenericBloomFilter_1(string config,bool ishash2); 
  void clear();
  void increment(const Address& addr);
  void decrement(const Address& addr);
  void merge(GenericBloomFilter_1 * other_filter);
  ...
  void printConfig(ostream& out) { out << "GenericBloomFilter_1" << endl; }

  // Destructor
  ~GenericBloomFilter_1();
...
extern inline
ostream& operator<<(ostream& out, const GenericBloomFilter_1& obj)
{
  obj.print(out);
  out << flush;
  return out;

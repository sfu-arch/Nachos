...
vector<Inst_info> g_ins_buffer_offload;
...
void write_inst_file(Trace_info* trace_info,Inst_info* i_info);
VOID EnterROI();
VOID ExitROI();
 ...
 VOID offload_funcAfter(ADDRINT ret)
{
  ...
  if(g_enable_instrument)
  {
    if(ret)
    {
      ...
      //Writing Chunk Begin Marker to the trace
      write_inst_file(trace_info,m);
      // write_inst_debugfile(trace_info,m);

      delete m;
      //------------------------------------------------------------------------------------------ 
      for(auto &ins_info: g_ins_buffer_offload)
      {

        Inst_info* ins_info_ptr = &ins_info;
        write_inst_file(trace_info, ins_info_ptr);
...
void write_inst_frm_ins_buffer(ADDRINT iaddr)
{
  ...
  // cout<<"static_inst:num_read_regs:"<<dec <<(uint32_t) static_inst->num_read_regs<<endl;
    ...
    //If ins belongs to offloaded func -- store in Buffer offlaod
    if(g_enable_offload)
      g_ins_buffer_offload.push_back(*i_info);
    else {


      write_inst_file(trace_info, i_info);
      if(g_inst_count%100)
        write_inst_debugfile(trace_info, i_info);

    // g_ins_buffer.clear();
    // g_ins_buffer.erase(iaddr);
    }
}
...
{
    //FIXME: ERROR
    memcpy(trace_info->trace_buf + trace_info->bytes_accumulated, i_info, sizeof(Inst_info));
    ...
    if (trace_info->bytes_accumulated == BUF_SIZE) {
        ...
        {
          cerr << "Normal Instr: " 
            << "ins_count: " << trace_info->inst_count 
            << "BUF_SIZE: "             << BUF_SIZE 
            <<" trace_stream_size:   "    << sizeof(trace_info->trace_stream)
            << endl;
          perror("Write failed: ");
          assert(0);
          return;
    ...
    //     dump_to_file(trace_info->debug_stream, i_info);
...
VOID Image(IMG img, VOID *v)
{
  ...
  if (RTN_Valid(offload_funcRtn))
  {
    ...
    // RTN_InsertCall(offload_funcRtn, IPOINT_BEFORE, (AFUNPTR)Arg1Before,
    //                IARG_ADDRINT, MALLOC,
  ...
  if (RTN_Valid(Ld_funcRtn))
  {
    ...
    // RTN_InsertCall(Ld_funcRtn, IPOINT_AFTER, (AFUNPTR)offload_funcAfter,
    //                IARG_FUNCRET_EXITPOINT_VALUE, IARG_END);
  ...
  if (RTN_Valid(St_funcRtn))
  {
    ...
    // RTN_InsertCall(St_funcRtn, IPOINT_AFTER, (AFUNPTR)offload_funcAfter,
    //                IARG_FUNCRET_EXITPOINT_VALUE, IARG_END);

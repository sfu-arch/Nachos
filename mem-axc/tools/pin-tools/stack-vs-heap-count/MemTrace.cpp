...
// #define ITERATION_THRESHOLD 5000000000

UINT64 iteration_no=0;
UINT64 ins_count=0;
UINT64 g_stack_read_no=0;
UINT64 g_stack_write_no=0;
UINT64 g_heap_read_no=0;
UINT64 g_heap_write_no=0;
...
VOID RecordMemRead(BOOL stackop)
{
  iteration_no++;
...
VOID RecordMemWrite( BOOL stackop)
{
  iteration_no++;
  // if(iteration_no > ITERATION_THRESHOLD) {
  //   Fini(0,0);
...
VOID Fini(INT32 code, VOID *v)
{
  ...
  fprintf(trace,"Stack Read: %zu \n", g_stack_read_no); 
  fprintf(trace,"Stack Write: %zu \n", g_stack_write_no); 
  fprintf(trace,"Heap Read: %zu \n", g_heap_read_no); 
  fprintf(trace,"Heap Write: %zu \n", g_heap_write_no); 

...
main(int argc, char* argv[])
{
    ...
    try {
     ...
     //   cout<<"memmap iters:"<<iterno<<endl;
     
     cout<<"Main: Loaded Iteration nos. \n";
     cerr<<"Main: Loaded Iteration nos. \n";

    // boost::filesystem::ifstream input_file(input_dotfile_path.c_str(),ios::in);
    // assert(input_file.is_open() && "Error! File couldn't open");


    //BoostGraph orig_graph(0);
    uint64_t memory_latency;
    int _mem_req_stack;
    _mem_req_stack = m_mem_req_stack;
    memory_latency= m_latency;
    uint32_t m_resource=256;
    Graph input_graph_c(m_resource, ldst_file_path.c_str(), input_dotfile_path.c_str(), stall_on_bloom,"LSB_Counting-" + to_string(bfp_size) + ":"+ to_string(128),bfp_ishash2, memory_latency, _mem_req_stack  );
    // std::ofstream output_file("in.dot",ios::out);
    // assert(output_file.is_open() && "Error! Output file couldn't create");
    // input_graph_c.printGraph(output_file);

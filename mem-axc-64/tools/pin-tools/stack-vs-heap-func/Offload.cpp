...
VOID mem_func_ins_LD(UINT32 nodeid, ADDRINT mem_addr)
{
  ...
  g_mem_set_offload.push_back({iteration_no, true, nodeid, mem_addr});

}


//ONLY runs when offloadedi region store function is called
VOID mem_func_ins_ST(UINT32 nodeid, ADDRINT mem_addr)
{
  if(!g_enable_instrument)
    return;
  ...
  ofload_mem++;
  g_mem_set_offload.push_back({iteration_no, false, nodeid, mem_addr});

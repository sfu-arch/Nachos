...
optimizebc()
{
    ...
    if [ "${out}" = "swaptions" ]; then
        UNROLL="-unroll-count=4"
    else
...
microworkload_trace()
{
    ...
    for n in {0..4}; do 
        ...
        env PATH=${LLVM_OBJ}:$PATH ${MWE} -fn=${2} -ExtractType::trace -simdfg -aa-edges -seq=path-seq-${n}.txt ${LIBS} -u=${HELPER_LIB} ${1} -o ${NAME}-mwe-${n} 2>> mwe.log
        if [ "$?" -ne 0 ]; then
            touch dun.goofed
        else
